...
export function NotebooksPage() {
	...
	const addUrl = async () => {
		...
		try {
			await apiPost(`/api/notebooks/${selected}/sources`, {
				url: url.trim(),
				wait: false,
			});
	...
	const ask = async () => {
		...
		try {
			const data = await apiPost(`/api/notebooks/${selected}/query`, {
		...
		<div className="space-y-6">
			...
			<Card className="glass p-4 flex flex-wrap gap-2 items-end">
				<div className="flex-1 min-w-[200px]">
					...
					<Input
						...
						placeholder="Fleet research topic"
					/>
				</div>
			...
			<div className="grid gap-4 lg:grid-cols-2">
				...
				<Card className="glass p-4 space-y-3">
					...
					{sources && (
						<pre className="text-xs bg-muted/40 p-2 rounded overflow-auto max-h-40">
